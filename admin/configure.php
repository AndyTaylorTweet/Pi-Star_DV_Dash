...
<?php
	if (isset($configdmrgateway['DMR Network 2']['Id'])) {
		if (strlen($configdmrgateway['DMR Network 2']['Id']) == 8) {
			$dmrPlusESSID = substr($configdmrgateway['DMR Network 2']['Id'], -1);
		} else {
			$dmrPlusESSID = "None";
		}
	} else {
		if (isset($configmmdvm['General']['Id'])) {
			if (strlen($configmmdvm['General']['Id']) == 8) {
				$dmrPlusESSID = substr($configmmdvm['General']['Id'], -1);
			} else {
				$dmrPlusESSID = "None";
			}
		} else {
...
<?php if ( $configmmdvm['POCSAG']['Enable'] == 1 ) { ?>
	...
	<td style="text-align: left;"><select name="pocsagServer">
      ...
      <tr>
        ...
        <td align="left"><input type="text" name="pocsagBlacklist" size="60" maxlength="200" value="<?php if (isset($configdapnetgw['General']['BlackList'])) { echo $configdapnetgw['General']['BlackList']; } ?>" /></td>
      </tr>
    </table>
	<div><input type="button" value="<?php echo $lang['apply'];?>" onclick="submitform()" /><br /><br /></div>
<?php } ?>	
