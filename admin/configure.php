...
<?php if (!empty($_POST)):
	...
	if (empty($_POST['confFREQtx']) != TRUE && empty($_POST['confFREQrx']) != TRUE ) {
	  ...
	  $newFREQOffset = $newFREQrx - $newFREQtx;
	  $newFREQOffset = substr_replace($newFREQOffset, '.', '1', 0);
	  $newFREQOffset = mb_strimwidth($newFREQOffset, 0, 6);
...
<?php
	...
	if ( !file_exists('/etc/timeserver.dissable') ) {
		...
		echo "<td align=\"left\" colspan=\"2\"><div class=\"switch\"><input id=\"toggle-timeAnnounce\" class=\"toggle toggle-round-flat\" type=\"checkbox\" name=\"confTimeAnnounce\" value=\"ON\" aria-hidden=\"true\" tabindex=\"-1\" ".$toggleDstarTimeAnnounceCr." /><label id=\"aria-toggle-timeAnnounce\" role=\"checkbox\" tabindex=\"0\" aria-label=\"D-Star Time Announcements\" aria-checked=\"false\" onKeyPress=\"toggleDstarTimeAnnounce()\" onclick=\"toggleDstarTimeAnnounce()\" for=\"toggle-timeAnnounce\"><font style=\"font-size:0px\">D-Star Time Annoucements</font></label></div></td>\n";
	}
    ?>
    </tr>
    <tr>
